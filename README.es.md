...
| **Cobertura de tests** | ~95% | parcial | parcial | parcial | parcial | parcial | parcial | parcial | **~98% (3000+ tests)** |
...
```bash
npm test              # Correr todos los 3000+ tests
...
Un minor grande. Destacados:

- 🎨 **Cuantización perceptual** — `rgbTo256Perceptual`, `nearestPerceptual`, `oklabDistance` (ΔE de Oklab — mata el banding vs RGB L2)
- 🖥️ **Pantalla alternativa gestionada** — `createScreen` (pantalla completa estilo vim/htop con restauración garantizada) — fundación TUI
- 🌈 **Gradientes spline** — `gradientColorSpline` (Catmull-Rom C¹, suave a través de multi-stops)
- ⏱️ **Cronómetro de vueltas** — `createStopwatch` (splits con etiqueta, `report()`, `slowest()`)
- 🧪 **+60 tests**

```js
import { createScreen, rgbTo256Perceptual, gradientColorSpline, createStopwatch } from 'ansimax';

await createScreen().run((s) => { s.moveTo(1,1); s.write('App pantalla completa'); });
rgbTo256Perceptual(128, 64, 32);              // xterm-256 más cercano por ΔE perceptual
gradientColorSpline(stops, 0.25);             // muestreo multi-stop suave
const sw = createStopwatch(); sw.lap('load'); // perfilar etapas
```

Drop-in replacement para `1.6.7`.
...
- 📏 **Detección de ancho Unicode** — `stringWidth`, `isFullWidth`, `isEmoji`, `isCombining`
- 📊 **Formatters de tasa** — `formatPercent`, `formatRate` (`"1.5 MB/s"`, `"1.2K req/s"`)
- 📈 **Suavizado EMA en ETA** — `createETA({ smoothing: 'ema' })` reacciona más rápido a cambios de velocidad
- 🔢 **Columnas numéricas auto-alineadas** — `ascii.table({ autoAlignNumbers: true })`
- 🧪 **+40 tests**
...
createETA({ total: 1000, smoothing: 'ema' });        // reacción más rápida
ascii.table(rows, { autoAlignNumbers: true });       // números a la derecha
```

Drop-in replacement para `1.6.4`.
...
- 📝 **Wrapping de celdas en `ascii.table`** — `wrap: true` envuelve celdas largas a múltiples líneas en vez de truncar
- 🧪 **Coverage hardening** — cerrados los gaps de branches de v1.4.8 (alcanzables → tests, inalcanzables → `istanbul ignore` documentado)
...
ascii.table([
  ['id', 'descripción'],
  ['1', 'una descripción bastante larga que se envuelve limpiamente en varias líneas'],
], { maxWidth: 40, wrap: true });
```

El column sizing corre primero (water-filling), luego las celdas se envuelven dentro de los anchos resultantes. Las filas crecen para caber su celda más alta. `wrap: false` (default) mantiene el comportamiento de ellipsis de v1.4.8.

Drop-in replacement para `1.4.8`.
