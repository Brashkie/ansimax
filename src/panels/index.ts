...
// ─────────────────────────────────────────────
//  v1.4.4 — Grid Areas (CSS grid-template-areas)
// ─────────────────────────────────────────────
...
export interface GridAreasOptions {
  /**
   * Symbolic layout matrix. Each cell holds a name; adjacent cells with
   * the same name merge into a single rectangular area. Use `'.'` for an
   * empty gap cell.
   *
   * ```js
   * areas: [
  ...
  /** Vertical gap between rows. Default `0`. */
  gapY?: number;
  /** Fix each cell to this width. If omitted, uses natural column widths. */
  cellWidth?: number | null;
  /** Fix each row to this height. If omitted, uses natural row heights. */
  cellHeight?: number | null;
  /** Content horizontal alignment inside cells. */
  alignX?: Alignment;
  /** Content vertical alignment inside cells. */
  alignY?: Alignment;
}

/**
 * Validate that every non-'.' name in the areas matrix forms a contiguous
 * rectangle. Throws with a helpful message on the first violation.
 *
...
export const _validateAreas = (areas: string[][]): AreaRect[] => {
  ...
  const boxes = new Map<string, { minR: number; maxR: number; minC: number; maxC: number; count: number }>();
  for (let r = 0; r < rows; r++) {
  ...
  for (const [name, box] of boxes) {
    ...
    // Verify every cell in the bounding box matches this name.
    //
    // This check is mathematically redundant given the count check above:
    // if `count === (h × w)` for the bounding box, every cell must belong
    // to this name (no room for another name to hide inside). Kept as
    // defense in depth — future changes to the count logic might make it
    // reachable, and the error message here is more specific.
 ...
 * ```
 *
 * Names in the matrix must form contiguous rectangles — non-rectangular
 * layouts throw. Use `'.'` for a gap cell (no block rendered there).
 *
 * @since 1.4.4
 */
export const gridAreas = (
  ...
  // Order blocks by (row, col) so grid's mark-and-pack sees them in
  // the same order they appear in the areas matrix. Any missing name
  // gets an empty block.
